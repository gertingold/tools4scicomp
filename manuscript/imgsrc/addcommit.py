...
for nr, (label, boxcolor, symbolcolor, status) in enumerate(
          (('working directory', color.hsb(0.87, 1, 0.6),
            color.rgb(0.6, 0, 0), 'modified'),
           ('staging area', color.hsb(0.2, 1, 0.6),
            color.rgb(0, 0.5, 0), 'staged'),
           ('repository (.git)', color.hsb(0.53, 1, 0.6),
            color.grey(0.3), 'committed'))):
    ...
    c.text(xmid, 0.2*(ht-htlabel), status, [text.halign.center, symbolcolor])

ht = ht-htlabel
for nr, operation in enumerate(('git add', 'git commit')):
    xmid = nr*(wd+hdist)+0.5*wd
    c.stroke(path.line(xmid+0.5*size+0.1, 0.55*ht,
    ...
    cop.insert(optext)
    c.insert(cop, [trafo.translate((nr+1)*(wd+hdist)-0.5*hdist, 0.55*ht)])
