...
def server(r, servercolor=color.rgb(0.5, 0.5, 0.8)):
    ...
    c.fill(path.circle(0, 0, r),
           [servercolor, trafo.scale(1, 0.5).translated(0, 0.5*r)])
    h = 2*r
...
for nr, (name, versions) in enumerate((('file 1', (0, 2, 4, 5)),
    ...
    for nver, (v1, v2) in enumerate(zip(versions[:-1], versions[1:])):
        ...
        cf.text(xll+0.5*width, yll+0.5*height, gethashstring(),
                [text.size(-4), text.halign.center, text.valign.middle,
                 color.grey(1)])
for n in range(nr_revisions):
    ...
    if n:
        yshift = 0.1
        cf.stroke(path.line(xcenter-0.33*dx, y+yshift,
                            xcenter-0.67*dx, y+yshift),
                  [deco.earrow])
