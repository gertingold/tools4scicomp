...
indexing with the arrray ``notdone``. An entry of ``True`` means that 
...
In the previous section we have seen hwo a program can be accelerated by means
...
actively developped at present. Therefore, we do not attempt a complete description
...
produce exectuable code for a function when it is called first. The compilation
step implies a certain investment of time but the function can be excuted faster
...
The following implementation of the code is not particulary well suited to
...
Like in NumPy and in contrast to Python, integers cannot become arbitrarly large.
...
.. figure:: img/numba_parallel.*
   ...
   of the number of threads on a CPU with four cores and hyerthreading.
