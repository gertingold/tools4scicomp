...
def server(r, servercolor=color.rgb(0.5, 0.5, 0.8), transparency=0):
    ...
    c.fill(path.circle(0, 0, r),
           [servercolor, color.transparency(transparency),
...
for phi in (0, 120, 240):
    c.stroke(path.curve(-sfak*sin(2*pi/3)+0.4, -0.5*sfak+0.15,
             ...
             [arrowcolor, deco.earrow.large, deco.barrow.large,
              style.linewidth.THick, trafo.rotate(phi)])
c.insert(server(0.5, color.hsb(0.5, 0.8, 0.5), 0.13))
c.text(0.8, 0.2, 'GitLab / GitHub server',
       [color.hsb(0.5, 0.8, 0.5), text.size.small])

c.writePDFfile()
c.writeGSfile(device="png16m", resolution=600) # use gimp to convert to png in case of transparency
                                               # problems
